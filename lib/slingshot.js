...
function setBandPoints(x, y, z, color)
{
    ...
    normals = [];
    weights = [];
    quadSet(0, y, z, -x, -y, -z, color);

    quadSet(x, y, z, 0, -y, -z, color);
...
function quad(a, b, c, d, x, y, z, nX, nY, nZ, color)
{
    ...
    for ( let i = 0; i < indices.length; ++i ) {
        if (vertices[indices[i]][0] < -1) {
            weights.push(vertexWeights[0]);
        }
        else if (vertices[indices[i]][0] > 1) {
            weights.push(vertexWeights[6]);
        }
        else {
