...
function getInterpolatedPoint(spline, nTime) {
    ...
    const q1 = euler2Quaternion(
        spline.pointsData[currentSegment + 1].rot.x,
        spline.pointsData[currentSegment + 1].rot.y,
        spline.pointsData[currentSegment + 1].rot.z
    );
    
    const q2 = euler2Quaternion(
        spline.pointsData[currentSegment + 2].rot.x,
        spline.pointsData[currentSegment + 2].rot.y,
        spline.pointsData[currentSegment + 2].rot.z
    );
    ...
    return {
        ...
        rotation: interpolatedQuat  // Return the quaternion directly
    };
}
...
function updateModelOnSpline(model, currentTime) {
    ...
    if (normalizedTime >= 1.0) {
        ...
        // Convert final rotation to quaternion and apply
        const finalRot = euler2Quaternion(
            finalPoint.rot.x,
            finalPoint.rot.y,
            finalPoint.rot.z
        );
        model.rotationQuaternion = finalRot;
    ...
    // Store the quaternion rotation directly in the model
    model.rotationQuaternion = interpolated.rotation;
